...
def opencv_wrapper(file_data, output_path, operation):
    try:
        ...
        if result is not None:
            cv2.imshow("image", result) #show result
            cv2.imwrite(new_path, result) #save result
            cv2.waitKey(0)
        else:
            logging.warning(
...
if __name__ == "__main__":
    ...
    #file_list = file_list(path) #use for folder only
    ...
    shutil.copy(path, "input/1.jpg")
    #os.replace(path, "input/1.jpg")
    file_list = file_list("input")


    #input mode
    mode = input("Please choose mode \n"+
                "1.shrink: It first downscales the images by a factor of 2 then it upscales by 4x using ESDR, this method helps if you have very noisy images \n"+
                "2.upscale: It skips the shrink and only upscales the images by a factor of 4\n"+
                "3.denoise: Runs Open CV fast denoising algorithm \n"+
                "Please enter your mode you want to use:")
    ...
    #logging.basicConfig(level=logging.DEBUG, format=FORMAT, handlers=[logging.FileHandler(f'{output_path}/output.log')])
    ...
    files_data = get_files_data(file_list, "input\\")
    #print(files_data)
